...
clearance      = float(input("クリアランスを指定: "))
endpitch       = float(input("終端孔ピッチを指定: "))
hole_size = int(input("孔径を指定"))
hole_endpitch_x = clearance + endpitch
...
# (1) shape_listの準備（selected_templateとして準備済み）
...
# ====== 角度指定をここで入力 ======
angle_deg = float(input("子H型鋼の角度を入力（度数）: "))
angle_rad = math.radians(angle_deg)
h10 = math.tan(angle_rad) * clearance

if angle_deg >= 0:
    # 正の角度 → 子梁を上げる方向
    pivot_x, pivot_y = w6, h10
else:
    # 負の角度 → 子梁を下げる方向
    pivot_x, pivot_y = 0, h10
...
for line in new_lines:
    x0, y0 = line[2], line[3]
    x1, y1 = line[4], line[5]
    x0r, y0r = rotate_point(x0, y0, angle_rad, pivot_x, pivot_y)
    x1r, y1r = rotate_point(x1, y1, angle_rad, pivot_x, pivot_y)
    rotated_new_lines.append([line[0], line[1], x0r, y0r, x1r, y1r] + line[6:])

# (4) rotate_point関数を用いてhole_listを回転（rotated_hole_listの作成）
rotated_hole_list = []
for hole in hole_list:
    if len(hole) == 9:
        x_rot, y_rot = rotate_point(hole[2], hole[3], angle_rad, pivot_x, pivot_y)
        x_rot_end, y_rot_end = rotate_point(hole[4], hole[5], angle_rad, pivot_x, pivot_y)
        rotated_hole_list.append([hole[0], hole[1], x_rot, y_rot, x_rot_end, y_rot_end] + hole[6:])
    elif len(hole) == 12 and hole[9] == "E":
        x_rot, y_rot = rotate_point(hole[2], hole[3], angle_rad, pivot_x, pivot_y)
        rotated_hole_list.append([hole[0], hole[1], x_rot, y_rot] + hole[4:])
    else:
        rotated_hole_list.append(hole)

# (5) 最後に上記の結果を全て統合して final_shape_list を生成（ここが最後）
