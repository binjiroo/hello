...
# 型鋼の名称
h_type = input("親H型鋼のサイズを指定:")#例400x200x8x13
sub_h_type = input("子H型鋼のサイズを指定:")#例250x125x6x9
hole_column = int(input("孔の列数"))
hole_pitch = int(input("孔ピッチ"))
gusset_name = input("ガセット名を指定:")

# 1."x" で分割して値部分のリストを得る
values_str = h_type.split("x") + sub_h_type.split("x")
...
shape_list.extend([
    [s1, s2, w1 + x_offset, h1 + y_offset, w2 + x_offset, h1 + y_offset, lc, lt, ly],
...
for i in range(hole_column):
    ...
    shape_list.append([
        s1, s2,
        w3 + x_offset, y_bolt + y_offset,
        w4 + x_offset, y_bolt + y_offset,
        lc, lt, ly
    ])

shape_list.append([
    s1, s2,
    w1 + x_offset, h3_end + y_offset,
    w2 + x_offset, h3_end + y_offset,
    lc, lt, ly
])

shape_list.extend([
    [s1, s2, w1 + x_offset, -h1 + y_offset, w2 + x_offset, -h1 + y_offset, lc, lt, ly],
    [s1, s2, w1 + x_offset, -h2 + 10 + y_offset, w2 + x_offset, -h2 + 10 + y_offset, lc, lt, ly],
    [s1, s2, w1 + x_offset, -h1 + y_offset, w1 + x_offset, -h2 + 10 + y_offset, lc, lt, ly],
    [s1, s2, w2 + x_offset, -h1 + y_offset, w2 + x_offset, -h2 + 10 + y_offset, lc, lt, ly],
    [999, 100, 50]
    ])
