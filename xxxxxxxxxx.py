...
import copy
#import instr
from math import atan2, degrees, radians, sin, cos, sqrt, pi
kivy.require('2.1.0')
...
class ShapeDrawer(Widget):
    def __init__(self, **kwargs):
        ...
        # 寸法表示の設定を追加
        self.dimension_scale = 1  # 1:10, 1:15, 1:20
        self.dimension_line_length = 50  # 寸法線の基本長さ
        self.show_dimensions = True  # 寸法表示のON/OFF
    ...
    def draw_line(self, start, end):
        instr = InstructionGroup()  # 描画指示のためのグループを作成
        instr.add(Line(points=[start[0], start[1], end[0], end[1]], stroke=self.current_color))
        shape = Shape('line', instr, self.current_color, start=start, end=end)
        self.shapes.append(shape)
        if self.show_dimensions:
            self.draw_dimensions(shape)
        # 現在のレイヤーに追加
        layer = self.layers[self.current_main_layer][self.current_sub_layer]
        layer['instructions'].add(instr)
        layer['shapes'].append(shape)
    ...
    def set_dimension_scale(self, scale):
        # 寸法線のスケール設定
        self.dimension_scale = scale

    def set_dimension_line_length(self, length):
        # 寸法線の長さ設定
        self.dimension_line_length = length

    def toggle_dimensions(self):
        # 寸法表示のON/OFF切替
        self.show_dimensions = not self.show_dimensions
        self.redraw_all_shapes()  # 全ての図形を再描画

    def redraw_all_shapes(self):
        # すべての図形を再描画し、寸法線の表示設定を更新
        for shape in self.shapes:
            shape.redraw()
            if self.show_dimensions:
                self.draw_dimensions(shape)

    def draw_dimensions(self, shape):
        instr = InstructionGroup()
        if shape.shape_type == 'line':
            self.add_line_dimension(shape, instr)
        elif shape.shape_type == 'rectangle':
            self.add_rectangle_dimension(shape, instr)
        elif shape.shape_type in ['circle', 'arc']:
            self.add_circle_dimension(shape, instr)
        self.canvas.add(instr)
        shape.dimensions = instr  # 寸法線を図形オブジェクトに保存

    def add_line_dimension(self, shape, instr):
        start, end = shape.kwargs['start'], shape.kwargs['end']
        mid_point = ((start[0] + end[0]) / 2, (start[1] + end[1]) / 2)
        length = self.get_distance(start, end)
        instr.add(Line(points=[start, end], dash_offset=10))
        instr.add(Label(text=f"{length:.2f} units", pos=mid_point))

    def add_rectangle_dimension(self, shape, instr):
        corners = shape.kwargs['corners']
        # 長さと面積の計算と表示
        width = self.get_distance(corners[0], corners[1])
        height = self.get_distance(corners[1], corners[2])
        area = width * height
        instr.add(Label(text=f"{width:.2f} x {height:.2f} units, Area: {area:.2f}", pos=shape.get_center()))

    def add_circle_dimension(self, shape, instr):
        center = shape.kwargs['center']
        radius = shape.kwargs['radius']
        circumference = 2 * pi * radius
        area = pi * (radius ** 2)
        instr.add(Label(text=f"Radius: {radius:.2f}, Circumference: {circumference:.2f}, Area: {area:.2f}", pos=center))
...
class ShapeApp(App):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.num_main_layers = 20  # メインレイヤーの数を設定
        self.num_sub_layers = 20   # サブレイヤーの数を設定
    def build(self):
        ...
        control_panel = BoxLayout(size_hint=(1, None), height=40, orientation='horizontal')
        ...
        # スナップモードトグルボタン
        self.snap_toggle = ToggleButton(text='OFF', size_hint=(0.1, 1))
        ...
        # 録画制御パネル
        record_panel = BoxLayout(size_hint=(1, None), height=130, orientation='vertical')

        # 寸法ボタン（具体的な実装は未定義）
        dimension_button = Button(text='寸法', size_hint=(1, 0.25))
        # dimension_button.bind(on_press=self.on_dimension_button) # 実装が必要です
        record_panel.add_widget(dimension_button)

        # 寸法表示ボタン（具体的な実装は未定義）
        Dimensioning_button = Button(text='表示', size_hint=(1, 0.25))
        # Dimensioning_button.bind(on_press=self.on_dimension_button) # 実装が必要です
        record_panel.add_widget(Dimensioning_button)

        # 録画開始ボタン
        self.record_button = ToggleButton(text='録画開始', size_hint=(1, 0.25))
        self.record_button.bind(on_press=self.on_record_toggle)
        record_panel.add_widget(self.record_button)

        # 再生ボタン
        replay_button = Button(text='再生', size_hint=(1, 0.))
        replay_button.bind(on_press=self.on_replay_button)
        record_panel.add_widget(replay_button)

        # レイヤーパネルの上に録画制御パネルを追加
        layer_panel.add_widget(record_panel)
