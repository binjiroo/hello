...
let DRAG_GEOM = null;
const DRAG_STORE_KEY = `splice_plate_drag_state::${location.pathname}${location.search}`;
...
function updateHoleGridInputs() {
  ...
  const outerText = buildHoleGridTotal(flangeX, flangeY);
  const innerText = buildHoleGridTotal(flangeX, Number.isFinite(flangeY) ? flangeY / 2 : NaN);
  const webText   = buildHoleGridTotal(webX, webY);
...
function estimateDimExtraMm(cfg, tier2Mm) {
  ...
  // 文字高さはだいたい 1.2em、端点ドットや線幅も少し足す（安全側）
  return gap + tier2 + tGap + fs * 1.2 + dotR + sw * 2;
}
...
function drawDimChainX(
  ...
  const sw0   = (cfg.strokeWidthMm  || 0.3) * s;
  const fs0   = (cfg.fontSizeMm     || 4)   * s;

  const strokeW = Math.max(cfg.minStroke ?? 0, sw0);
  const fontSz  = Math.max(cfg.minFontSize ?? 0, fs0);
...
function buildFlangeYDimPitchChainMm(plateWidthMm, rowPitchMm, holeCountY) {
  ...
  // ラベル：まとめず “1ピッチずつ”
  const labels = [];
  labels.push(Math.round(edge));
  for (let j = 0; j < nY - 1; j++) labels.push(Math.round(rowPitchMm));
  labels.push(Math.round(edge));
...
function saveDragStateToStorage() {
  try {
    ...
    localStorage.setItem(DRAG_STORE_KEY, JSON.stringify(payload));
  } catch (err) {
    console.warn("[splice] saveDragStateToStorage failed:", err);
  }
}

function loadDragStateFromStorage() {
  try {
...
function bindCanvasDrag(svg) {
  ...
  window.addEventListener("mousemove", (ev) => {
    ...
    st.offsetX = DRAG_SESSION.startOffsetX + adjVx;
    st.offsetY = DRAG_SESSION.startOffsetY + adjVy;
...
function initSpliceDrawingCanvas() {
  ...
  frame.setAttribute("width",  String(drawingWidthMm  - margin * 2));
  frame.setAttribute("height", String(drawingHeightMm - margin * 2));
...
function isHoleCrossEnabled() {
  ...
  return cb ? cb.checked : holeCenterCrossEnabled;
}
...
function renderHSectionZoomIndicator() {
  ...
  if (!hasR) {
    ...
    webRect.setAttribute("stroke-width", "0.4");
  ...
  midLine.setAttribute("stroke-width", "0.2");
...
if (Number.isFinite(plateWidthMm) && flangeYPitchOk && flangeYCountOk) {
  ...
  edgeMarginMm = (plateWidthMm - spanYmm) / 2;

  yCentersMm = [];
  nY = n;

  const startYmm = edgeMarginMm;
  for (let j = 0; j < nY; j++) {
...
const MAX_CENTER_PAD_DRAW = 8; // ????????????????????
const autoPadTopRaw = (AUTO_CENTER_Y ? freeY / 2 : 0);
const autoPadTop = Math.max(0, Math.min(autoPadTopRaw, MAX_CENTER_PAD_DRAW));
...
if (y > bottomLimit + 0.01) {
  ...
  plateRect.setAttribute("stroke-width", "0.6");
  ...
  if (xPitchOk && xCountOk && diaOk) {
    ...
    const dimCfg = getDimCfg();
          dimCfg.frameGapMm = 6;       // ★枠線から 6mm 離す（まずは 6 推奨）
          dimCfg.labelSafePadMm = 3;   // ★ラベル見積りに 3mm 足す
  ...
  if (Number.isFinite(webPlateDrawLength) && Number.isFinite(webPlateDrawHeight)) {
    ...
    webPlateRect.setAttribute("stroke-width", "0.5");
  ...
  if (gWeb && webColPitchOk && webCountXOk && webDiaOk) {
    ...
    if (webRowPitchOk && webCountYOk) {
      ...
      const startY = (webPlateHeightMm - spanY) / 2;
      for (let j = 0; j < nY; j++) {
  ...
  if (hText) {
    const label = document.createElementNS(SVG_NS, "text");
    label.setAttribute("x", String(drawingWidthMm / 2));
    label.setAttribute("y", String(plateOuterY + plateDrawHeight / 2));
    label.setAttribute("text-anchor", "middle");
