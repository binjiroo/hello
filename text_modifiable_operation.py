...
class TextModifiableOperation(Screen):
    def __init__(self, **kwargs):
        super(TextModifiableOperation, self).__init__(**kwargs)
        self.build_ui()
    ...
    def build_ui(self):
        ...
        self.stats_label = Label(text='', font_size=24, size_hint_y=None, height=30)
        left_layout.add_widget(self.stats_label)

        # フォントファイルのパスを設定する
        font_path = 'C:\Windows\Fonts\meiryo.ttc'
        self.text_input = TextInput(size_hint_y=None, multiline=True, height=600, font_name=font_path, input_type='text', write_tab=False)
...
class MyApp(App):
    def build(self):
        sm = ScreenManager()
        sm.add_widget(TextModifiableOperation(name='text_modifiable_operation'))
        return sm
