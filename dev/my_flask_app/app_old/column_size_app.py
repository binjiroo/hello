...
from .config import (
    column_steel_r_mapping as steel_r_mapping,
)  # ← これで定義済みマッピングを読み込む

steel_sizes = list(steel_r_mapping.keys())

column_size_bp = Blueprint(
    "column_size", __name__, template_folder="templates/column_size"
)
...
@column_size_bp.route("/", methods=["GET", "POST"])
def index():
    raise Exception("ここが実行されているか確認")
    current_app.logger.info("index() 実行")
    filenames = [
        "JW_OPT4.DAT",
        "JW_OPT4B.DAT",
        "JW_OPT4C.DAT",
