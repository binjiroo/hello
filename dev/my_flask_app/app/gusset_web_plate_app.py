...
from .config import (
    gusset_web_plate_mapping as steel_r_mapping,
)  # ← これで定義済みマッピングを読み込む
from .size_dat_utils import build_dat_line, insert_leader_follow_rows, is_checked

steel_sizes = list(steel_r_mapping.keys())

gusset_web_plate_bp = Blueprint(
    "gusset_web_plate", __name__, template_folder="templates/gusset_web_plate"
)
...
@gusset_web_plate_bp.route("/", methods=["GET", "POST"])
def index():
    filenames = [
        "JW_OPT4.DAT",
        "JW_OPT4B.DAT",
    ...
    if request.method == "POST":
        ...
        frame_rows = insert_leader_follow_rows(frame_rows, leader_follow)

        # 2) 追加計算で “だけ” 出したい 7 行
        append_only_rows = name_rows + frame_rows

        # 3) （新規時のみ）ボルト列とトップラインを作る
        bolt_rows = []
        if hole_column >= 1:
            ...
            for i in range(hole_column):
                ...
                bolt_rows.append([
                    s1, s2,
                    w3 + x_off, y_bolt + y_off,
                    w4 + x_off, y_bolt + y_off,
                    lc, lt, ly,
                ])
            ...
            top_line_row = [
                s1, s2,
                w1 + x_off, top_line_y + y_off,
                w2 + x_off, top_line_y + y_off,
                lc, lt, ly,
            ]
        else:
            top_line_row = []

        tail_rows = [[999, 100, 50]]  # 終端情報（一覧表示倍率など）
        ...
        lines = [build_dat_line(row) for row in shape_list]
        new_result = "\n".join(lines)
