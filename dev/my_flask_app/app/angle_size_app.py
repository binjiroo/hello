...
import math
from .config import (
    angle_steel_r_mapping as steel_r_mapping,
)  # ← これで定義済みマッピングを読み込む

from .size_dat_utils import build_dat_line, insert_leader_follow_rows, is_checked
steel_sizes = list(steel_r_mapping.keys())

angle_size_bp = Blueprint(
    "angle_size", __name__, template_folder="templates/angle_size"
)
...
@angle_size_bp.route("/", methods=["GET", "POST"])
def index():
    filenames = [
        "JW_OPT4.DAT",
        "JW_OPT4B.DAT",
