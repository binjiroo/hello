...
from app.data.steel_mappings import cannel_steel_r_mapping as steel_r_mapping
...
bp = Blueprint("cannel_size", __name__, url_prefix="/cad/cannel_size", template_folder="templates")
...
def index():
    ...
    if request.method == "POST":
        ...
        if steel_name not in steel_r_mapping:
            error_message = "選択したチャンネル鋼サイズが見つかりません。"
            defaults = get_defaults()
            return render_template(
                "cannel_size/index.html",
                filenames=filenames,
