...
from . import bp
from app.data.steel_mappings import gusset_web_plate_mapping as steel_r_mapping
...
def build_gusset_web_plate_result(values, action="new", previous_result=""):
    if action == "clear":
        return "", ""
    ...
    try:
        ...
        clearance = int(state["clearance"] or 0)
        end_hole = int(state["end_hole_pitch"] or 40)
        ...
        filtered = [value for index, value in enumerate(vals) if index not in (0, 3, 4, 7)]
    ...
    frame_rows = [
        ...
        [s1, s2, -w5 + x_off, h2 + y_off, -w5 + x_off, h2 + 10 + y_off, 10000, 4, ly, mating_gusset],
    ]
    frame_rows = _insert_leader_follow_rows(frame_rows, leader_follow)
    ...
    if hole_column >= 1:
        ...
        for index in range(hole_column):
            y_bolt = h4 + start + index * hole_pitch
            last_bolt_y = y_bolt
            bolt_rows.append(
                [s1, s2, w3 + x_off, y_bolt + y_off, w4 + x_off, y_bolt + y_off, lc, lt, ly]
            )

        top_line_y = last_bolt_y + 40 if last_bolt_y is not None else h1 + 40
        top_line_row = [s1, s2, w1 + x_off, top_line_y + y_off, w2 + x_off, top_line_y + y_off, lc, lt, ly]
...
def generate():
    ...
    result, error_msg = build_gusset_web_plate_result(
        payload,
        action=action,
